defmodule GT.AccountSupervisor do
  @moduledoc """
  Supervisor to handle the creation of dynamic `GT.Account` processes using a 
  ...
  `GT.Account.start_link` function.
  ...
  is attached to a running `GT.Account` process.

  If the `account_id` is not present in the registry, it will create a new `GT.Account` 
  ...
  @doc """
  Determines if a `GT.Account` process exists, based on the `account_id` provided.

  Returns a boolean.

  ## Example
      iex> GT.AccountSupervisor.account_process_exists?(6)
      false
  """
  ...
  @doc """
  Returns the count of `GT.Account` processes managed by this supervisor.

  ## Example
      iex> GT.AccountSupervisor.account_process_count
      0
  """
  ...
  def get_all_account_widgets_ordered do
    account_ids() |> Enum.map(&(%{ account_id: &1, widgets_sold: GT.Account.widgets_ordered(&1) }))
  end
  ...
  def init(_) do
    children = [
      worker(GT.Account, [], restart: :permanent)
    ]
